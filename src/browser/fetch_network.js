 ...
 */
function FetchNetworkAdapter(bus, options)
{
    options = options || {};
    let adapter = this;
    this.bus = bus;
    this.router_mac = options.router_mac || new Uint8Array([82, 84, 0, 1, 2, 3]);
    this.router_ip = options.ip || new Uint8Array([192, 168, 86, 1]);
    this.tcp_conn = {};

    // 'https://corsproxy.io/?' for example
    this.cors_proxy = options.cors_proxy ? options.cors_proxy : false;
